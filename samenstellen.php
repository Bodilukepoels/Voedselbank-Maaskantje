    ...
    if (isset($voedselpakketName)) {
        try {
            $updateStmt = $conn->prepare("UPDATE gezinnen SET pakket = :pakket WHERE id = :id");
            $updateStmt->bindParam(':pakket', $voedselpakketName);
            $updateStmt->bindParam(':id', $currentFamilyId);
            $updateStmt->execute();
            header("Location=overzichtgezinnen");
        } catch (PDOException $e) {
            $errorMessage = "Error updating gezinnen table: " . $e->getMessage();
        }
    }
    ...
    if (isset($_SESSION['gezin'])) {
        $currentFamilyId = $_SESSION['gezin'];
    } else {
...
$currentFamilyId = null;
if (isset($_GET['id'])) {
    $currentFamilyId = $_GET['id'] ?? null;

}

foreach ($gezinnen as $gezin) : ?>
    <?php if ($gezin['id'] == $currentFamilyId) : ?>
        <tr>
            <td><?= $gezin['naam']; ?></td>
